...
const generateGromacsColabNotebook = (nodes: Node[], edges: Edge[]): string => {
...
export default function VisualBuilder() {
  ...
  const hasGromacsSimulate = React.useMemo(
    () => nodes.some((n) => n.type === "simulate" && (n.data as Record<string, unknown> | undefined)?.engine === "gromacs"),
    [nodes],
  );

  const handleDownloadGromacsColab = useCallback(() => {
    try {
      const nb = generateGromacsColabNotebook(nodes, edges);
      const blob = new Blob([nb], { type: "application/x-ipynb+json" });
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = url;
      link.download = `gromacs_gpu_colab_${makeTimestampSuffix()}.ipynb`;
      link.click();
      window.URL.revokeObjectURL(url);
      toast.success(`GROMACS GPU Colab notebook downloaded: ${link.download}`, {
