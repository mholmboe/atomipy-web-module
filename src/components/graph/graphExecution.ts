  ...
  sorted.forEach((id, index) => {
    ...
    switch (n.type) {
      ...
      case "simulate": {
        ...
        pythonCode += `        print(f"  Initial potential energy: {_md_init_pe:,.1f} kJ/mol")\n`;
        pythonCode += `        # Pre-MD relaxation. A freshly built/solvated system carries small local\n`;
        pythonCode += `        # forces (close contacts from solvation, organic merge, lattice write-out).\n`;
        pythonCode += `        # GPU/OpenCL platforms often tolerate them, but the CPU platform does not\n`;
        pythonCode += `        # and the first MD steps diverge to NaN. A short minimization makes the run\n`;
        pythonCode += `        # platform-independent without changing the equilibrium physics.\n`;
        pythonCode += `        print("  Relaxing initial structure (pre-MD energy minimization)...")\n`;
        pythonCode += `        try:\n`;
        pythonCode += `            simulation.minimizeEnergy(tolerance=10*unit.kilojoule_per_mole/unit.nanometer, maxIterations=2000)\n`;
        pythonCode += `            _md_min_pe = simulation.context.getState(getEnergy=True).getPotentialEnergy().value_in_unit(unit.kilojoule_per_mole)\n`;
        pythonCode += `            print(f"  Post-minimization potential energy: {_md_min_pe:,.1f} kJ/mol")\n`;
        pythonCode += `        except Exception as _min_err:\n`;
        pythonCode += `            print(f"  Pre-MD minimization warning: {_min_err}")\n`;
        pythonCode += `        # Initialize velocities to the target temperature (avoids a cold-start kick).\n`;
        pythonCode += `        simulation.context.setVelocitiesToTemperature(${temp}*unit.kelvin)\n`;
