  ...
  sorted.forEach((id, index) => {
    ...
    switch (n.type) {
      ...
      case "simulate": {
        ...
        if (engine === "gromacs") {
          // GROMACS path (gmx binary, GMXRC, or install dir). Defaults to 'gmx' on PATH
          // (works on Colab/standard installs); set the field for a custom install.
          const gmxSpec = getString(data, "gmxPath", "gmx").trim() || "gmx";
          ...
          pythonCode += `    # Collect any organic GAFF/OpenFF .itp files that reached here so they are\n`;
          pythonCode += `    # #included (their atomtypes are hoisted into the .top by write_dummy_system_top).\n`;
          ...
          pythonCode += `    if _dummy_org_itps: print(f"[dummy] GROMACS: {len(_dummy_org_itps)} organic .itp(s) included (atomtypes hoisted): {_dummy_org_itps}")\n`;
      ...
      case "viewer": {
        ...
        if (inTraj) {
          ...
          pythonCode += `                    _curr_model.append(_l)\n`;
          pythonCode += `            \n`;
          pythonCode += `            # Downsample the IN-BROWSER trajectory animation only (the full trajectory is\n`;
          pythonCode += `            # always written to disk / included in the download). The whole animation is\n`;
          pythonCode += `            # inlined in ONE SSE message and held in the tab as text + a parsed structure,\n`;
          pythonCode += `            # so the hard limit is browser MEMORY (heap ~ atoms x frames x ~55 bytes), NOT\n`;
          pythonCode += `            # the renderer — NGL (the default) plays thousands of frames smoothly. Budget the\n`;
          pythonCode += `            # inlined size to keep the tab safe (~44 MB in -> ~0.4 GB heap) and cap frames at\n`;
          pythonCode += `            # ~1000. Both tunable via ATOMIPY_VIEWER_FRAME_BUDGET (atom-lines) and\n`;
          pythonCode += `            # ATOMIPY_VIEWER_MAX_FRAMES. For truly unlimited frames, load the native .xtc/.dcd\n`;
          pythonCode += `            # from the bundle directly into NGL (streams frames instead of inlining text).\n`;
          pythonCode += `            _n_full = len(_models)\n`;
          pythonCode += `            _atoms_per_frame = max(1, sum(1 for _l in _models[0] if _l.startswith(('ATOM', 'HETATM')))) if _models else 1\n`;
          pythonCode += `            _v_budget = int(os.environ.get('ATOMIPY_VIEWER_FRAME_BUDGET', '800000'))\n`;
          pythonCode += `            _v_cap = int(os.environ.get('ATOMIPY_VIEWER_MAX_FRAMES', '1000'))\n`;
          pythonCode += `            _max_out_models = max(20, min(_v_cap, _v_budget // _atoms_per_frame))\n`;
