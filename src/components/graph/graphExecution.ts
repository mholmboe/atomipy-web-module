    ...
    switch (n.type) {
      ...
      case "viewer": {
        ...
        if (inTraj) {
          ...
          pythonCode += `            # Size-aware cap: the whole trajectory is inlined in ONE SSE message and then\n`;
          pythonCode += `            # parsed/bonded/animated by 3Dmol, so the cost scales with atoms x frames, not\n`;
          pythonCode += `            # frames alone. Budget the animation by total atom-lines (a 3000-atom water box\n`;
          pythonCode += `            # at 100 frames is ~300k lines / tens of MB -> slow over the Colab tunnel).\n`;
          ...
          pythonCode += `            _max_out_models = max(10, min(100, 60000 // _atoms_per_frame))\n`;
