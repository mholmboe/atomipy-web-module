    ...
    switch (n.type) {
      ...
      case "viewer": {
        ...
        if (inTraj) {
          ...
          pythonCode += `            # Show up to 100 frames in the in-browser animation. The whole trajectory is\n`;
          pythonCode += `            # inlined in ONE SSE message and then parsed/bonded/animated by the viewer, so\n`;
          pythonCode += `            # cost scales with atoms x frames. We target 100 frames and only back off for\n`;
          pythonCode += `            # very large systems (atom-line budget) so a huge box can't stall the stream.\n`;
          pythonCode += `            # The full trajectory is always written to disk / included in the download.\n`;
          ...
          pythonCode += `            _max_out_models = max(10, min(100, 500000 // _atoms_per_frame))\n`;
