    ...
    switch (n.type) {
      ...
      case "viewer": {
        ...
        if (inTraj) {
          ...
          pythonCode += `        # Out-of-band trajectory (TEST): a large multi-frame PDB inlined in one SSE\n`;
          pythonCode += `        # message is slow over the Colab tunnel. Instead, reference the file by name\n`;
          pythonCode += `        # (it's in the result bundle) and let the viewer fetch it after the run via\n`;
          pythonCode += `        # /download-result/<token>/file/<name>. Single-frame files stay inline.\n`;
          pythonCode += `        _has_models = False\n`;
          pythonCode += `        with open(_traj_file, 'r', encoding='utf-8', errors='replace') as _f:\n`;
          pythonCode += `            for _l in _f:\n`;
          pythonCode += `                if _l.startswith('MODEL'):\n`;
          pythonCode += `                    _has_models = True\n`;
          pythonCode += `                    break\n`;
          pythonCode += `        if _has_models:\n`;
          pythonCode += `            with open(_traj_file, 'r', encoding='utf-8', errors='replace') as _f:\n`;
          pythonCode += `                _nframes = sum(1 for _l in _f if _l.startswith('MODEL'))\n`;
          pythonCode += `            print(f"Trajectory ready ({_nframes} frames) - the viewer will fetch it from the result bundle.", flush=True)\n`;
          pythonCode += `            _vis_pdb_str = "@FILE:" + os.path.basename(_traj_file)\n`;
          pythonCode += `        else:\n`;
          pythonCode += `            # errors='replace': trajectory PDBs (e.g. from gmx trjconv) can carry a stray\n`;
          pythonCode += `            # non-UTF8 byte; never let the viewer crash on it.\n`;
          pythonCode += `            with open(_traj_file, 'r', encoding='utf-8', errors='replace') as _f:\n`;
