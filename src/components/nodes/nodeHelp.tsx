...
export const NODE_HELP: Record<string, NodeHelp> = {
  ...
  simulate: {
    ...
    features: [
      ...
      "Parameter sweep (GROMACS): run this stage once per value of a parameter — temperature, pressure, any custom .mdp key (e.g. ref_t, tau_t, init-lambda-state — set on the auto .mdp), or a __SWEEP__ placeholder in a full custom .mdp. 'Independent' runs each value from the input structure; 'Sequential' continues each from the previous window (e.g. annealing). Writes sweep_summary.csv and streams a metric-vs-value curve to a connected Data Plotter that fills in per window; the final window's structure passes downstream.",
      "Free energy (λ-FEP): pick 'Free energy' in the sweep, set the moleculetype to decouple and the vdw-lambdas (one window per value). It runs a window per λ state, writes each dH/dλ file, then runs gmx bar to get ΔG ± error (fep_result.json) with a cumulative-ΔG-vs-λ plot. Best on local/Colab; needs decent sampling per window.",
      "Umbrella sampling (PMF): pick 'Umbrella sampling' in the sweep and give two pull-group selections (resname=MOL / molid=1). It runs an SMD pull along the COM distance, picks windows ~spacing nm apart, runs a restrained MD per window, then gmx wham to build the potential of mean force (pmf_profile.csv / pmf_result.json) with a PMF-vs-ξ plot. Feed it an equilibrated structure (chain EM/NVT first); best on local/Colab.",
    ],
    theory: [
